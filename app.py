...
def research_report_content(report_id):
    ...
    elif report_id.startswith('oracle__'):
        # oracle__<relative_path_with___as_separator>
        rel = report_id[len('oracle__'):].replace('__', '/')
        oracle_dir = Path.home() / 'Documents' / 'Trading Vault' / '03_Stock_Analysis' / 'ORACLE'
        full_path = (oracle_dir / (rel + '.md')).resolve()
        oracle_resolved = oracle_dir.resolve()
        if oracle_resolved not in full_path.parents or not full_path.is_file():
            return jsonify({'error': 'report not found'}), 404
        from flask import Response
        return Response(full_path.read_text(encoding='utf-8', errors='replace'),
                        mimetype='text/plain',
                        headers={'Access-Control-Allow-Origin': '*'})
    else:
        folder = report_id
