...
def api_account():
    ...
    # Principal: set ALPACA_PRINCIPAL in .env, or it auto-calculates from transfer history
    VERIFIED_PRINCIPAL = float(os.environ.get('ALPACA_PRINCIPAL', '0') or '0')
    if VERIFIED_PRINCIPAL == 0:
        # Auto-calculate from Alpaca transfer history
        try:
            transfers = alpaca('/v2/account/activities', {'activity_types': 'CSD,CSW', 'page_size': 100})
            if isinstance(transfers, list):
                VERIFIED_PRINCIPAL = sum(float(t.get('net_amount', t.get('amount', 0)) or 0)
                                        for t in transfers if float(t.get('net_amount', t.get('amount', 0)) or 0) > 0)
        except Exception:
            VERIFIED_PRINCIPAL = float(acct.get('last_equity', equity) or equity)
